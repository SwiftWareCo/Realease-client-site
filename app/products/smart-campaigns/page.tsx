                ...
                <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", width: "100%", position: "relative", zIndex: 1 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
                        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                            <Link href="/" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "0.85rem", color: "#10B981", textDecoration: "none", marginBottom: 24, fontWeight: 600 }}>
                                <ArrowLeft size={16} /> Back to Home
                            </Link>

                            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 20px", background: "rgba(16,185,129,0.06)", borderRadius: 50, marginBottom: 24, border: "1px solid rgba(16,185,129,0.12)" }}>
                                <Sparkles size={14} style={{ color: "#10B981" }} />
                                <span style={{ fontSize: "0.8rem", fontWeight: 600, color: "#10B981" }}>Smart Campaigns</span>
                            </div>
                            ...
                            <h1 style={{ fontSize: "clamp(2rem, 4vw, 3.2rem)", fontWeight: 800, lineHeight: 1.1, fontFamily: "var(--font-dm-sans)", letterSpacing: "-0.04em", marginBottom: 20 }}>
                                Outreach That{" "}
                                <span style={{ background: "linear-gradient(135deg, #10B981, #059669, #4F46E5)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                                    Thinks for You.
                                </span>
                            </h1>
                        ...
                        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>
                            <div style={{ background: "linear-gradient(135deg, #F0FDF9, #EEF2FF)", borderRadius: 20, padding: 24, border: "1px solid rgba(16,185,129,0.1)", boxShadow: "0 24px 48px rgba(16,185,129,0.08)" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
                                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#10B981", animation: "pulse-soft 2s ease-in-out infinite" }} />
                                    <span style={{ fontSize: "0.75rem", fontWeight: 600, color: "#10B981" }}>Campaign Activity Feed</span>
                                </div>
                                <div style={{ background: "white", borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
                                    {campaignPreview.map((line, i) => (
                                        <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.25 }} style={{ padding: "10px 14px", borderRadius: 10, background: line.type === "Stats" ? "rgba(16,185,129,0.04)" : "#FAFBFE", border: line.type === "Stats" ? "1px solid rgba(16,185,129,0.12)" : "1px solid #F1F5F9" }}>
                ...
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
                        {features.map((f, i) => (
                            ...
                            >
                                <div style={{ width: 48, height: 48, borderRadius: 12, background: "rgba(16,185,129,0.06)", display: "flex", alignItems: "center", justifyContent: "center", color: "#10B981", marginBottom: 16 }}>{f.icon}</div>
