...
export default function CalendarPage() {
    ...
    const typeColors: Record<string, string> = { showing: "#4F46E5", meeting: "#F59E0B", call: "#10B981" };
    ...
    return (
        <>
            ...
            <section style={{ minHeight: "100vh", display: "flex", alignItems: "center", paddingTop: 100, paddingBottom: 60, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ position: "absolute", top: -100, right: -100, width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(6,182,212,0.10), transparent 70%)", filter: "blur(60px)", pointerEvents: "none" }} />

                <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 24px", width: "100%", position: "relative", zIndex: 1 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
                        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                            <Link href="/" style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: "0.85rem", color: "#06B6D4", textDecoration: "none", marginBottom: 24, fontWeight: 600 }}>
                                <ArrowLeft size={16} /> Back to Home
                            </Link>

                            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 20px", background: "rgba(6,182,212,0.06)", borderRadius: 50, marginBottom: 24, border: "1px solid rgba(6,182,212,0.12)" }}>
                                <CalendarDays size={14} style={{ color: "#06B6D4" }} />
                                <span style={{ fontSize: "0.8rem", fontWeight: 600, color: "#06B6D4" }}>Synced Calendar</span>
                            </div>
                            ...
                            <h1 style={{ fontSize: "clamp(2rem, 4vw, 3.2rem)", fontWeight: 800, lineHeight: 1.1, fontFamily: "var(--font-dm-sans)", letterSpacing: "-0.04em", marginBottom: 20 }}>
                                Every Meeting.{" "}
                                <span style={{ background: "linear-gradient(135deg, #06B6D4, #0891B2, #4F46E5)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
                                    Perfectly Synced.
                                </span>
                            </h1>
                            ...
                            <div style={{ display: "flex", gap: 24, marginBottom: 32 }}>
                                {[{ value: "95%", label: "Auto-Scheduled" }, { value: "Zero", label: "Conflicts" }, { value: "2", label: "Integrations" }].map((s) => (
                                    <div key={s.label}>
                                        <p style={{ fontSize: "1.5rem", fontWeight: 800, color: "#06B6D4", fontFamily: "var(--font-dm-sans)" }}>{s.value}</p>
                        ...
                        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3 }}>
                            <div style={{ background: "linear-gradient(135deg, #ECFEFF, #E0F2FE)", borderRadius: 20, padding: 24, border: "1px solid rgba(6,182,212,0.1)", boxShadow: "0 24px 48px rgba(6,182,212,0.08)" }}>
            ...
            <section style={{ padding: "100px 24px", background: "#FAFBFE" }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <div style={{ textAlign: "center", marginBottom: 64 }}>
                        <p style={{ fontSize: "0.8rem", fontWeight: 700, color: "#06B6D4", textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 12 }}>Capabilities</p>
                        <h2 style={{ fontFamily: "var(--font-dm-sans)" }}>Scheduling Made <span className="gradient-text">Effortless</span></h2>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }}>
                        {features.map((f, i) => (
                            <motion.div key={f.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ background: "white", borderRadius: 16, padding: 28, border: "1px solid #E2E8F0", transition: "box-shadow 0.3s, transform 0.3s" }}
                                onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 12px 40px rgba(6,182,212,0.08)"; e.currentTarget.style.transform = "translateY(-4px)"; }}
                                onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.transform = "translateY(0)"; }}
                            >
                                <div style={{ width: 48, height: 48, borderRadius: 12, background: "rgba(6,182,212,0.06)", display: "flex", alignItems: "center", justifyContent: "center", color: "#06B6D4", marginBottom: 16 }}>{f.icon}</div>
            ...
            <section style={{ padding: "100px 24px", background: "#0F172A" }}>
                <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
                    ...
                    <button onClick={openCalendly} style={{ padding: "16px 40px", background: "linear-gradient(135deg, #06B6D4, #0891B2)", color: "white", border: "none", borderRadius: 50, fontSize: "1rem", fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 24px rgba(6,182,212,0.3)" }}>
